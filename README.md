# Java Summary

面向 Java 学习与工程实践的结构化知识库，覆盖 **初级 -> 中级 -> 高级**，目标是做到：
- 路径清晰：知道先学什么、后学什么
- 复习高效：能快速定位高频考点
- 表达可用：可用于学习复盘、项目汇报与面试口述

## 从这里开始

1. 先看总入口：[`docs/README.md`](./docs/README.md)
2. 再选一种阅读方式：
   - 按学习顺序：[`docs/01-按学习顺序索引.md`](./docs/01-按学习顺序索引.md)
   - 按面试频率：[`docs/02-按面试频率索引.md`](./docs/02-按面试频率索引.md)
   - 按知识专题：[`docs/03-按专题索引.md`](./docs/03-按专题索引.md)
3. 查看大纲基线：[`docs/10-Java后端-知识大纲-v1.md`](./docs/10-Java后端-知识大纲-v1.md)

## 学习路线图

```mermaid
flowchart LR
    A["L1 初级: 会用并能解释基础"] --> B["L2 中级: 能定位问题并优化"]
    B --> C["L3 高级: 能设计架构并做权衡"]
    C --> D["综合实战: 高频题 + 项目题 + 系统设计题"]
```

## 目录结构

- 总索引：[`docs/README.md`](./docs/README.md)
- 学习路线：[`docs/00-总览-学习路线.md`](./docs/00-总览-学习路线.md)
- 调研提炼：[`docs/11-GitHub调研提炼-2026-03-05.md`](./docs/11-GitHub调研提炼-2026-03-05.md)
- 六周执行计划：[`docs/12-六周学习执行计划.md`](./docs/12-六周学习执行计划.md)
- 题库编号规则：[`docs/13-面试题库编号与复习规则.md`](./docs/13-面试题库编号与复习规则.md)
- 最终汇总：[`docs/14-最终汇总-从基础到高级完成版.md`](./docs/14-最终汇总-从基础到高级完成版.md)
- P0/P1 速记卡：[`docs/15-P0-P1速记卡-可打印版.md`](./docs/15-P0-P1速记卡-可打印版.md)
- 30 分钟模拟脚本：[`docs/16-30分钟模拟面试脚本.md`](./docs/16-30分钟模拟面试脚本.md)
- 个人台词模板：[`docs/17-个人面试实战台词模板.md`](./docs/17-个人面试实战台词模板.md)
- 详细度审计：[`docs/18-文档详细度审计-2026-03-05.md`](./docs/18-文档详细度审计-2026-03-05.md)
- 练习标准答案示例：[`docs/19-练习任务标准答案示例-L1-L3.md`](./docs/19-练习任务标准答案示例-L1-L3.md)
- 跨章节串联学习地图：[`docs/20-跨章节串联学习地图-L1-L2-L3.md`](./docs/20-跨章节串联学习地图-L1-L2-L3.md)
- 压测容量与阈值模板：[`docs/21-压测容量与回归阈值模板.md`](./docs/21-压测容量与回归阈值模板.md)
- 分层索引：
  - [`docs/L1-初级/README.md`](./docs/L1-初级/README.md)
  - [`docs/L2-中级/README.md`](./docs/L2-中级/README.md)
  - [`docs/L3-高级/README.md`](./docs/L3-高级/README.md)
- 示例代码：[`examples/README.md`](./examples/README.md)
- 规则约定：[`AGENTS.md`](./AGENTS.md)

## 阅读建议

- 每次学习一个主题，遵循「结论 -> 原理 -> 场景 -> 问答 -> 代码」。
- 每周至少完成：
  - 3 个能完整解释原理的知识点
  - 10 道可口述的面试题
  - 2 个可运行示例

## 状态说明

- `TODO`：已规划，待编写
- `DOING`：编写中
- `DONE`：已完成并可复习

## 当前进度

- L1：4 个核心模块已完成
- L1 子章节：19/19 已完成
- L2：4 个核心模块已完成
- L2 子章节：18/18 已完成
- L3：4 个核心模块已完成
- L3 子章节：16/16 已完成
- 详细化改造：知识点正文 65/65 已覆盖前置知识、练习任务、可运行代码
- 深挖案例补强：L2/L3 正文 42/42 已覆盖完整案例 Walkthrough
- 第三轮配套模板：练习标准答案 + 跨章节学习地图 + 压测容量模板已补齐
- 第四轮章级题解：知识点正文 65/65 已覆盖错答示例 -> 修正答法 -> 打分差异
- 可运行示例代码：`examples/`（含 L1/L2/L3）
